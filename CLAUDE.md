...
- **`api/index.py`** (repo root) is the only file Vercel's Python runtime needs - it just does `from backend.main import app`. Vercel has native FastAPI framework support: it auto-detects a `FastAPI` instance named `app` at conventional entrypoints (including `api/index.py`) and routes **every path** to it automatically - no WSGI adapter (e.g. Mangum) and no manual rewrite needed.
- **`vercel.json` deliberately has NO rewrites** - only a `functions` entry for `maxDuration`. An earlier version of this file had a manual rewrite (`"/(.*)" -> "/api/index"`) to force every path to the function, which seemed necessary before finding Vercel's native FastAPI support. That rewrite actively broke routing in production: a Vercel platform change started forwarding the literal *rewritten destination path* to the app instead of the original request path, so every route (including `/`) 404'd identically. If you ever see every route return the same 404/error, suspect a rewrite fighting Vercel's own routing before suspecting the FastAPI app.
...
- **`requires-python` in `pyproject.toml` must stay `>=3.12`**, not `>=3.10` - Vercel's Python builder installs its own `vercel-runtime` helper package into the same uv-managed venv as our dependencies, and that package requires Python 3.12+. A lower floor lets uv resolve `uv.lock` against 3.10/3.11 (which builds fine locally), then fails on Vercel with an unsatisfiable-dependency error at build time.
- **`.vercelignore` matters independently of `.gitignore`**: `vercel --prod` run from the CLI uploads the local working directory directly rather than pulling from GitHub, and does **not** automatically respect `.gitignore` for that upload. Without `.vercelignore` explicitly excluding `data/`, local test conversations from the file-storage backend ended up bundled into a real deployment and got served back as if they were production data. `.env*`/`.venv`/etc. need the same explicit treatment.
- **Upstash env var names aren't guaranteed to be `UPSTASH_REDIS_REST_URL`/`_TOKEN`**: connecting Upstash via the Vercel dashboard's Storage tab produced `UPSTASH_REDIS_REST_KV_REST_API_URL`/`UPSTASH_REDIS_REST_KV_REST_API_TOKEN` instead (the `KV_REST_API_*` part is the legacy "Vercel KV" naming the integration still uses internally, prefixed with whatever the database resource is named). `storage.py`'s `_UPSTASH_ENV_VAR_PAIRS` checks a few known variants rather than assuming the plain names `Redis.from_env()` expects - if Redis still isn't picked up after connecting a database, run `vercel env ls` and check what it actually named things.
- **Env vars marked "Sensitive" (the default for anything added via `vercel env add` or a marketplace integration) are write-only** - `vercel env pull` returns the literal string `"[SENSITIVE]"` instead of the real value, by design, for both CLI and dashboard. There is no way to read the value back out once set; if you need to move a value to a differently-named var, you have to re-enter it from the original source (e.g. the Upstash dashboard), not copy it from Vercel.
